...
<nav {{ $attributes->merge(['class' => '-mb-px flex flex-nowrap gap-x-1 overflow-x-auto border-b border-line']) }}
     aria-label="Sections">
        ...
        <a href="{{ $tab['url'] }}"
           ...
           class="flex shrink-0 items-center gap-1.5 border-b-2 px-3 py-2.5 text-sm font-medium transition
            ...
            @if (($tab['count'] ?? null) !== null)
                <span class="rounded-full bg-elevated px-1.5 text-xs tabular-nums text-ink-2">{{ $tab['count'] }}</span>
            @endif
        </a>
    @endforeach
</nav>
