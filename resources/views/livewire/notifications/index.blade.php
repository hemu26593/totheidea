<div>
    ...
    <x-ui.card title="Triggers" subtitle="The nine reminders, and what each is waiting on." class="mb-5">
        <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            @foreach ($triggers as $trigger)
                <div class="rounded-md px-3 py-2 ring-1 ring-inset {{ $trigger['blocked'] ? 'bg-amber-50 ring-amber-200' : 'bg-slate-50 ring-slate-200' }}">
                    <p class="font-mono text-xs font-medium text-slate-800">{{ $trigger['key'] }}</p>

                    @if ($trigger['blocked'])
                        <p class="mt-1 text-xs text-amber-800">{{ $trigger['blocked'] }}</p>
                    @else
                        <p class="mt-1 text-xs text-slate-500">Active.</p>
                    @endif
                </div>
            @endforeach
        </div>
    ...
    <x-ui.table :headings="['Trigger', 'Channel', 'Recipient', 'Scheduled', 'Status', 'Attempts', '>']">
        @forelse ($dispatches as $dispatch)
            <tr wire:key="dispatch-{{ $dispatch->id }}" class="hover:bg-slate-50">
                ...
                <x-ui.td>
                    ...
                    @if ($dispatch->error)
                        <p class="mt-0.5 max-w-xs truncate text-xs text-rose-700">{{ $dispatch->error }}</p>
                    @endif
                </x-ui.td>
